...
for i in bin/{nodetool,sstableloader,cqlsh,cqlsh.py} tools/bin/{cassandra-stress,cassandra-stressd,sstabledump,sstablelevelreset,sstablemetadata,sstablerepairedset} bin/{local_file_key_generator,configuration_encryptor}; do
    bn=$(basename $i)
    install -m755 $i "$rprefix"/share/cassandra/bin
    if ! $nonroot; then
        echo "$thunk" > "$rusr"/bin/$bn
